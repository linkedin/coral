...
public abstract class CoralSparkViewCatalog<T extends TableCatalog & SupportsNamespaces>
  ...
  public View loadView(Identifier identifier) {
    ...
    if (table == null || !table.getTableType().equalsIgnoreCase(TableType.VIRTUAL_VIEW.name())) {
      throw new NoSuchElementException(String.format("View doesn't exist %s", tbl));
        ...
        final DataType catalystSchema = org.apache.spark.sql.avro.SchemaConverters.toSqlType(
            new Schema.Parser().setValidate(false).setValidateDefaults(false)
                .parse(avroSchema.toString())).dataType();
        ...
        return new View() {
          ...
          public Map<String, String> properties() {
            return ImmutableMap.of(
                CatalogTable.VIEW_REFERRED_TEMP_FUNCTION_NAMES(),
                jsonArrayString(
                    coralSpark.getSparkUDFInfoList().stream()
                        .map(SparkUDFInfo::getFunctionName)
  ...
  private void registerUDFs(List<SparkUDFInfo> sparkUDFInfoList, String viewName) {
    ...
    final List<FunctionResource> resolvedFunctionResources = mavenDependencies.stream()
        .map(f -> new FunctionResource(FunctionResourceType.fromString("jar"), f.toString()))
        .collect(Collectors.toList());
          ...
          case HIVE_CUSTOM_UDF:
            if (isSparkUdf(className)) {
              ...
              sparkSession.sessionState()
                  .functionRegistry()
                  .registerFunction(new FunctionIdentifier(functionName), (Seq<Expression> children) -> {
                    try {
              ...
              LOG.info(String.format("Registered %s as Spark UDF with class %s for view %s.", functionName,
                  className, viewName));
            } else {
              sparkSession.sessionState()
                  .catalog()
              ...
              LOG.info(String.format("Registered %s as Hive UDF with class %s for view %s.", functionName,
                  className, viewName));
            }
            break;
          case TRANSPORTABLE_UDF:
            try {
              Utils.classForName(className, true, false)
  ...
  private boolean isSparkUdf(String className) {
    try {
      ...
      if (genericParameterTypes.length == 1 && genericParameterTypes[0] instanceof ParameterizedType) {
        ...
        if (actualTypeArguments.length == 1 && actualTypeArguments[0].getTypeName().equals("org.apache.spark.sql.catalyst.expressions.Expression")) {
          return true;
        }
      }
