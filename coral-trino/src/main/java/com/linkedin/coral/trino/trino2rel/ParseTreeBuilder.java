...
public class ParseTreeBuilder extends AstVisitor<SqlNode, ParserVisitorContext> {
    ...
    private static final ImmutableMap<String, TimeUnit> TIME_UNIT_MAP =
            ImmutableMap.<String, TimeUnit>builder().put(Extract.Field.YEAR.name(), TimeUnit.YEAR).put(Extract.Field.QUARTER.name(), TimeUnit.QUARTER)
                    .put(Extract.Field.MONTH.name(), TimeUnit.MONTH).put(Extract.Field.WEEK.name(), TimeUnit.WEEK).put(Extract.Field.DAY.name(), TimeUnit.DAY)
                    .put(Extract.Field.DAY_OF_MONTH.name(), TimeUnit.DAY).put(Extract.Field.DAY_OF_WEEK.name(), TimeUnit.DOW).put(Extract.Field.DOW.name(), TimeUnit.DOW)
                    .put(Extract.Field.DAY_OF_YEAR.name(), TimeUnit.DOY).put(Extract.Field.DOY.name(), TimeUnit.DOY).put(Extract.Field.HOUR.name(), TimeUnit.HOUR)
    ...
    private SqlIdentifier convertQualifiedName(QualifiedName name, SqlParserPos pos) {
        return CalciteUtil.createSqlIdentifier(pos,
                new ArrayList<>(name.getOriginalParts().stream().map(Identifier::getValue).collect(Collectors.toList())).toArray(new String[0]));
    }
    ...
    protected SqlNode visitWithQuery(WithQuery node, ParserVisitorContext context) {
        ...
        if (node.getColumnNames().isPresent()) {
            columns = toSqlNodeList(
                    node.getColumnNames().get(), context, pos);
        }
    ...
    protected SqlNode visitSimpleCaseExpression(SimpleCaseExpression node, ParserVisitorContext context) {
        ...
        SqlNode defaultValue =
                node.getDefaultValue().isPresent() ? process(node.getDefaultValue().get(), context) : CalciteUtil.createLiteralNull(ZERO);
    ...
    protected SqlNode visitIfExpression(IfExpression node, ParserVisitorContext context) {
        SqlParserPos pos = getPos(node);
        SqlNode defaultValue =
                node.getFalseValue().isPresent() ? process(node.getFalseValue().get(), context) : CalciteUtil.createLiteralNull(ZERO);
    ...
    protected SqlNode visitSortItem(SortItem node, ParserVisitorContext context) {
        ...
        SqlNode ordering = node.getOrdering().equals(SortItem.Ordering.DESCENDING) ?
                DESC.createCall(pos, operand) : operand;
    ...
    protected SqlNode visitJoin(Join node, ParserVisitorContext context) {
        ...
        if (node.getCriteria().isPresent()) {
            ...
            if (joinCriteria instanceof JoinOn) {
                ...
                condition =
                        toSqlNodeList(((JoinUsing) joinCriteria).getColumns(), context, getPos(node));
            }
        }
    ...
    protected SqlNode visitWindowDefinition(WindowDefinition node, ParserVisitorContext context) {
        ...
        return SqlWindow.create(visitIdentifier(node.getName(), context),
                window.getRefName(), window.getPartitionList(), window.getOrderList(), createLiteralBoolean(window.isRows(), ZERO),
                window.getLowerBound(), window.getUpperBound(), createLiteralBoolean(window.isAllowPartial(), ZERO), window.getParserPosition());
    }
    ...
    protected SqlNode visitGenericDataType(GenericDataType node, ParserVisitorContext context) {
        if (node.getArguments().isEmpty()) {
            return SqlTypeUtil.convertTypeToSpec(sqlTypeFactory.createSqlType(SqlTypeName.valueOf((node.getName().getValue().toUpperCase()))));
        }
        switch (node.getName().getValue().toUpperCase()) {
            ...
            case "BINARY":
                ...
                return SqlTypeUtil.convertTypeToSpec(sqlTypeFactory.createSqlType(SqlTypeName.valueOf(node.getName().getValue().toUpperCase()), len));
            case "ARRAY":
    ...
    protected SqlNode visitIntervalDataType(IntervalDayTimeDataType node, ParserVisitorContext context) {
        return new SqlIntervalQualifier(TIME_UNIT_MAP.get(node.getFrom().name()), TIME_UNIT_MAP.get(node.getTo().name()), getPos(node));
    }
