...
public class ToDateOperatorTransformer extends SqlCallTransformer {
  ...
  protected SqlCall transform(SqlCall sqlCall) {
    ...
    SqlCall sqlCastCall = new SqlCastFunction().createCall(SqlParserPos.ZERO, sqlCall.getOperandList().get(0),
        SqlLiteral.createCharString("TIMESTAMP", SqlParserPos.ZERO));
    newOperands.add(sqlCastCall);
