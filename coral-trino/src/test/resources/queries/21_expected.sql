...
from `lineitem` as `l3`
where `l3`.`l_orderkey` = `l1`.`l_orderkey` and `l3`.`l_suppkey` <> `l1`.`l_suppkey` and `l3`.`l_receiptdate` > `l3`.`l_commitdate`) and `s_nationkey` = `n_nationkey` and `n_name` = 'nation name'
group by `s_name`
