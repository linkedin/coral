...
from (select `ws_ext_sales_price` as `ext_price`, `ws_sold_date_sk` as `sold_date_sk`, `ws_item_sk` as `sold_item_sk`, `ws_sold_time_sk` as `time_sk`
from `web_sales`
...
from `store_sales`
where `d_date_sk` = `ss_sold_date_sk` and `d_moy` = 11 and `d_year` = 1999) as `tmp`
