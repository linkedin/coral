...
group by `i_item_id`) (select `ss_items`.`item_id`, `ss_item_rev`, cast(`ss_item_rev` / (cast(`ss_item_rev` as decimal(16, 7)) + `cs_item_rev` + `ws_item_rev`) / 3 * 100 as decimal(7, 2)) as `ss_dev`, `cs_item_rev`, cast(`cs_item_rev` / (cast(`ss_item_rev` as decimal(16, 7)) + `cs_item_rev` + `ws_item_rev`) / 3 * 100 as decimal(7, 2)) as `cs_dev`, `ws_item_rev`, cast(`ws_item_rev` / (cast(`ss_item_rev` as decimal(16, 7)) + `cs_item_rev` + `ws_item_rev`) / 3 * 100 as decimal(7, 2)) as `ws_dev`, (`ss_item_rev` + `cs_item_rev` + `ws_item_rev`) / 3 as `average`
from `ss_items`
