...
where `i_current_price` between asymmetric 0.99 and 1.49 and `i_item_sk` = `cs_item_sk` and `cs_warehouse_sk` = `w_warehouse_sk` and `cs_sold_date_sk` = `d_date_sk` and cast(`d_date` as date) between asymmetric cast('2000-03-11' as date) - interval '30' day and cast('2000-03-11' as date) + interval '30' day
