...
public class ParseTreeBuilderTest {
    ...
    public Iterator<Object[]> getSupportedSql() {
        return ImmutableList.<List<String>>builder()
                .add(ImmutableList.of("select * from foo", "SELECT * FROM `FOO`"))
                ...
                .add(ImmutableList.of("select extract(day from x), extract(dow from x) from y", "SELECT EXTRACT(DAY FROM `X`), EXTRACT(DOW FROM `X`) FROM `Y`"))
                .add(ImmutableList.of("select 1 + 13 || '15' from foo", "SELECT `concat`(1 + 13, '15') FROM `FOO`"))
                .add(ImmutableList.of("select x is distinct from y from foo where a is not distinct from b", "SELECT `X` IS DISTINCT FROM `Y` FROM `FOO` WHERE NOT `A` IS DISTINCT FROM `B`"))
                ...
                .add(ImmutableList.of("select x[cast(10 * sin(x) as bigint)] from my_table", "SELECT `X`[CAST(10 * SIN(`X`) AS BIGINT)] FROM `MY_TABLE`"))
                ...
                .add(ImmutableList.of("select x from unnest(array[1, 2, 3]) t(x)", "SELECT `X` FROM UNNEST(ARRAY[1, 2, 3]) AS `T` (`X`)"))
                .add(ImmutableList.of("select * from users cross join unnest(friends)", "SELECT * FROM `USERS` CROSS JOIN UNNEST(`FRIENDS`)"))
                .add(ImmutableList.of("select id, friend from users cross join unnest(friends) t(friend)", "SELECT `ID`, `FRIEND` FROM `USERS` CROSS JOIN UNNEST(`FRIENDS`) AS `T` (`FRIEND`)"))
                .add(ImmutableList.of("select * from unnest(t.my_array) with ordinality", "SELECT * FROM UNNEST(`T`.`MY_ARRAY`) WITH ORDINALITY"))
                .add(ImmutableList.of("select * from unnest(array[1, 2, 3]) with ordinality", "SELECT * FROM UNNEST(ARRAY[1, 2, 3]) WITH ORDINALITY"))
                .add(ImmutableList.of("select x from unnest(array[1, 2, 3]) with ordinality t(x)", "SELECT `X` FROM UNNEST(ARRAY[1, 2, 3]) WITH ORDINALITY AS `T` (`X`)"))
                .add(ImmutableList.of("select * from users cross join unnest(friends) with ordinality", "SELECT * FROM `USERS` CROSS JOIN UNNEST(`FRIENDS`) WITH ORDINALITY"))
                .add(ImmutableList.of("select id, friend from users cross join unnest(friends) with ordinality t(friend)", "SELECT `ID`, `FRIEND` FROM `USERS` CROSS JOIN UNNEST(`FRIENDS`) WITH ORDINALITY AS `T` (`FRIEND`)"))
                .add(ImmutableList.of("select count(*) x from src group by k, v", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY `K`, `V`"))
                .add(ImmutableList.of("select count(*) x from src group by cube (k, v)", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY CUBE(`K`, `V`)"))
                .add(ImmutableList.of("select count(*) x from src group by rollup (k, v)", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY ROLLUP(`K`, `V`)"))
                .add(ImmutableList.of("select count(*) x from src group by grouping sets ((k, v))", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY GROUPING SETS((`K`, `V`))"))
                .add(ImmutableList.of("select count(*) x from src group by grouping sets ((k, v), (v))", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY GROUPING SETS((`K`, `V`), `V`)"))
                .add(ImmutableList.of("select count(*) x from src group by grouping sets (k, v, k)", "SELECT COUNT(*) AS `X` FROM `SRC` GROUP BY GROUPING SETS(`K`, `V`, `K`)"))
                .add(ImmutableList.of("select depname, empno, salary , count(*) over () , avg(salary) over (partition by depname) , rank() over (partition by depname order by salary desc) , sum(salary) over (order by salary rows unbounded preceding) , sum(salary) over (partition by depname order by salary rows between current row and 3 following) , sum(salary) over (partition by depname range unbounded preceding) , sum(salary) over (rows between 2 preceding and unbounded following) , lag(salary, 1) ignore nulls over (partition by depname) , lag(salary, 1) respect nulls over (partition by depname) from emp", "SELECT `DEPNAME`, `EMPNO`, `SALARY`, COUNT(*) OVER (), AVG(`SALARY`) OVER (PARTITION BY `DEPNAME`), RANK() OVER (PARTITION BY `DEPNAME` ORDER BY `SALARY` DESC), SUM(`SALARY`) OVER (ORDER BY `SALARY` ROWS UNBOUNDED_PRECEDING), SUM(`SALARY`) OVER (PARTITION BY `DEPNAME` ORDER BY `SALARY` ROWS BETWEEN CURRENT_ROW AND `FOLLOWING`(3)), SUM(`SALARY`) OVER (PARTITION BY `DEPNAME` RANGE UNBOUNDED_PRECEDING), SUM(`SALARY`) OVER (ROWS BETWEEN `PRECEDING`(2) AND UNBOUNDED_FOLLOWING), LAG(`SALARY`, 1) OVER (PARTITION BY `DEPNAME`), LAG(`SALARY`, 1) OVER (PARTITION BY `DEPNAME`) FROM `EMP`"))
                .add(ImmutableList.of("with a (id) as (with x as (select 123 from z) select * from x)    , b (id) as (select 999 from z) select * from a join b using (id)", "WITH `A` (`ID`) AS (WITH `X` AS (SELECT 123 FROM `Z`) (SELECT * FROM `X`)), `B` (`ID`) AS (SELECT 999 FROM `Z`) (SELECT * FROM `A` INNER JOIN `B` USING (`ID`))"))
                ...
                .add(ImmutableList.of("select TOTALPRICE \"my price\" from \"$MY\" \"ORDERS\"", "SELECT `TOTALPRICE` AS `MY PRICE` FROM `$MY` as `ORDERS`"))
                .add(ImmutableList.of("select * from foo tablesample system (10) join bar tablesample bernoulli (30) on a.id = b.id", "SELECT * FROM `FOO` TABLESAMPLE SYSTEM(10.000000149011612) INNER JOIN `BAR` TABLESAMPLE BERNOULLI(30.000001192092896) ON `A`.`ID` = `B`.`ID`"))
                .add(ImmutableList.of("select * from foo tablesample system (10) join bar tablesample bernoulli (30) on not(a.id > b.id)", "SELECT * FROM `FOO` TABLESAMPLE SYSTEM(10.000000149011612) INNER JOIN `BAR` TABLESAMPLE BERNOULLI(30.000001192092896) ON NOT `A`.`ID` > `B`.`ID`"))
                ...
                .add(ImmutableList.of("SELECT * FROM table1 WHERE a >= ALL (VALUES 2, 3, 4)", "SELECT * FROM `TABLE1` WHERE `A` >= ALL (VALUES 2, 3, 4)"))
                .add(ImmutableList.of("SELECT * FROM table1 WHERE a <> ANY (SELECT 2, 3, 4)", "SELECT * FROM `TABLE1` WHERE `A` <> SOME (SELECT 2, 3, 4)"))
                .add(ImmutableList.of("SELECT * FROM table1 WHERE a = SOME (SELECT id FROM table2)", "SELECT * FROM `TABLE1` WHERE `A` = SOME (SELECT `ID` FROM `TABLE2`)"))
    ...
    public void testProductCases() throws Exception {
        ...
        for (String file : files) {
            ...
            assertEquals(loadResource(format("product_test_cases_expected/%s_expected.sql", file.split("\\.")[0])), node.toString().toLowerCase());
        }
    }
