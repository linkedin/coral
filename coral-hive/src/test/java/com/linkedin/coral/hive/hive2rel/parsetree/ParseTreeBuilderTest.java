...
public class ParseTreeBuilderTest {
  ...
  /**
   * Validates that reserved keywords can be used as unquoted table aliases in FROM clauses.
   * This is needed because Hive's CBO automatically adds the table name as an alias when
   * persisting view definitions. When the table name is a reserved keyword (e.g., collection,
   * group, comment, role), the parser must accept it as a valid alias.
   * See BDP-70589.
   */
