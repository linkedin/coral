...
/**
 * Static implementation of FunctionRegistry that has hard-coded list of all
 ...
 *
 * Note that Coral maintains a copy of StaticCoralFunctionRegistry for read only at run time.
 * For individual query, we create a copy of function registry in a RelConTextProvider object.
 */
public class StaticCoralFunctionRegistry implements FunctionRegistry {
  ...
  static {
    ...
    FUNCTION_MAP.put("in", CoralFunction.IN);

    //addFunctionEntry("in", SqlStdOperatorTable.IN);

    // operators
    addFunctionEntry("rlike", CoralRLikeOperator.RLIKE);
    addFunctionEntry("regexp", CoralRLikeOperator.REGEXP);
    ...
    FUNCTION_MAP.put("when", CoralFunction.WHEN);
    FUNCTION_MAP.put("case", CoralFunction.CASE);
    FUNCTION_MAP.put("between", CoralFunction.BETWEEN);
    ...
    addFunctionEntry("named_struct", CoralNamedStructFunction.NAMED_STRUCT);
    ...
    addFunctionEntry("json_tuple", CoralJsonTupleOperator.JSON_TUPLE);
   ...
   * @return list of matching Functions or empty collection.
   */
  @Override
  ...
  private static void addCastOperatorEntries() {
    ...
    for (String f : castFunctions) {
      FUNCTION_MAP.put(f, CoralFunction.CAST);
    }
  }
