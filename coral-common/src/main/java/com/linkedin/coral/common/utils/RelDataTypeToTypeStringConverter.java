...
/**
 * Transforms a RelDataType to a type string such that it is parseable and semantically correct.
 * Some example of type strings for a RelDataType are as follows:
 *
 * Example 1:
 * RelDataType:
 ...
 */
public class RelDataTypeToTypeStringConverter {
  private RelDataTypeToTypeStringConverter() {
  }

  public RelDataTypeToTypeStringConverter(boolean convertUnionTypes) {
    this.convertUnionTypes = convertUnionTypes;
  }

  /**
   * If true, Coral will convert single uniontypes back to native uniontype representation. This is necessary
   * because some engines have readers that unwrap single uniontypes to just the underlying data type, causing
   * the loss of information that the column was originally a uniontype. This can be problematic when calling
    ...
    switch (relDataType.getSqlTypeName()) {
      ...
      default:
        throw new RuntimeException(String.format(
            "Unhandled RelDataType %s in Converter from RelDataType to DataType", relDataType.getSqlTypeName()));
    }
  }

  /**
   * Build a struct/row string with format:
  ...
  private static String buildStructDataTypeString(RelRecordType relRecordType) {
    ...
    // Convert single uniontypes as structs back to native representation
  ...
  /**
   * Build an array string with format:
   *   array<[field_type]>
  ...
  /**
   * Build a map string with format:
